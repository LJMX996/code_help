  ...
  if (llhelp) {
      ...
      for (let i = 0; i < Code_num; i++) {
          if (cookiesArr[i]) {
			  ...
			  llgetshare=false;
              await GetCollect();
              let chouShareCodes = newShareCodes;
			  if(llgetshare){
				  await $.wait(5000);
				  lnrun++;				  
			  }
...
async function doTenWaterAgain() {
  ...
  if (totalEnergy >= ($.farmInfo.farmUserPro.treeTotalEnergy - $.farmInfo.farmUserPro.treeEnergy)) {
    ...
    if (isFruitFinished) {
	  ...
	  if ($.isNode() && WP_APP_TOKEN_ONE) {
			console.log($.name, `【提醒⏰】${$.farmInfo.farmUserPro.name}已可领取\n【领取步骤】京东->我的->东东农场兑换京东红包,可以用于京东app的任意商品.`, `${$.UserName}`);
		}
    }
...
async function turntableFarm() {
  ...
  if ($.initForTurntableFarmRes.code === '0') {
    ...
    for (let code of chouShareCodes) {
...
async function showMsg() {
  ...
  if ($.ctrTemp) {
    ...
    if ($.isNode()) {
      ...
      // console.log(`${$.name} - 账号${$.index} - ${$.nickName}`, `${subTitle}\n${message}`);
    }
  } else {
    $.log(`\n${message}\n`);
  }
