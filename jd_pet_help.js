    ...
    try {
        ...
        if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
            ...
            if (!$.petInfo.goodsInfo) {
                ...
                if ($.isNode())
                    console.log(`${$.name} - ${$.index} - ${$.nickName || $.UserName}`, `【提示】京东账号${$.index}${$.nickName || $.UserName}\n暂未选购新的商品`);
                return
            }
            ...
            if ($.petInfo.petStatus === 5) {
                ...
                if ($.isNode()) {
                    console.log(`${$.name} - 账号${$.index} - ${$.nickName || $.UserName}奖品已可领取`, `京东账号${$.index} ${$.nickName || $.UserName}\n${$.petInfo.goodsInfo.goodsName}已可领取`);
                }
                return
